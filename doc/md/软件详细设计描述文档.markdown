...
| 王灿灿 | 2017-11-14| 修改mfdoc模块设计图 | v1.2.8 |
...
|`SalesMakeBillController.fresh()`|刷新界面|
|`SalesMakeBillController.logout()`|退出界面|
...
|`PurchaseMakeBillController.fresh()`|刷新界面|
|`PurchaseMakeBillController.logout()`|退出界面|
...
|`MemberManageController.find()`|查找客户|
|`MemberManageController.deleteInit()`|删除客户后刷新界面|
|`MemberManageController.initfind()`|查找客户后刷新界面|
|`MemberManageController.initInsert()`|存储客户后刷新界面|
<br/>

4.2.11　promotionui模块

(1)模块概述

promotionui模块承担的需求参见需求规格说明文档功能需求几相关非功能需求。
Promotionui模块的职责及接口参见软件体系结构描述文档表。

(2)整体结构

根据体系结构的设计，我们将系统分为展示层、业务逻辑层、数据层。每一层之间为了增加灵活性，我们会添加接口。比如展示层和业务逻辑层之间，我们添加businesslogicservice.promotionblservice.PromotionBLService接口。

Promotionui模块的设计如图所示。

<center>**promotionui模块各个类的职责如下所示**</center>


|模块|职责|
| --- | --- |
|PromotionMakingController|促销界面的模型对象|
|PromotionMemberController|等级促销界面的模型对象|
|PrpmotionSpecialController|特价包界面的模型对象|
|PrpmotionSumController|总额促销界面的模型对象|
...
| --- | --- |
|服务名|服务|
|`PromotionMakingController.choosePromotion())`|选择促销策略|
|`PromotionMakingController.initData(UserVO vo)`|初始化界面数据|
|`PromotionMakingController.choiceInit(UserVO vo)`|初始化界面数据|
...
　　　　saledata模块承担的需求参见需求规格说明文档功能需求几相关非功能需求。
　　　　Saledata模块的职责及接口参见软件体系结构描述文档表。

　　　　(2)整体结构

　　　　根据体系结构的设计，我们将系统分为展示层、业务逻辑层、数据层。每一层之间为了增加灵活性，我们会添加接口。比如业务逻辑层和数据层之间，我们添加dataservice.saledataservice.SaleDataService接口。AccountPO是作为销售记录的持久化对象被添加到设计模型中去的。

　　saledata模块各个类的职责如下所示
<br/>

|模块|职责|
| --- | --- |
|SaleData|销售(退货)的模型对象，拥有销售单据的各类数据以及方法|
...
| --- | --- |
|服务名|服务|
|`SaleDataService.find(String keywords, FindSaleType type)`|根据keywords查找单一持久化对象|
|`SaleDataService.insert(SalePO po)`|新建单一持久化对象|
|`SaleDataService.delete(String id)`|删除单一持久化对象|
|`SaleDataService.update(SalePO po)`|更新单一持久化对象|
|`SaleDataService.show()`|返回数据库中所有持久化对象|
<br/>

4.3.8　Purchasedata模块

　　　　(1)模块概述

　　　　purchasedata模块承担的需求参见需求规格说明文档功能需求几相关非功能需求。
　　　　Purchasedata模块的职责及接口参见软件体系结构描述文档表。

　　　　(2)整体结构

　　　　根据体系结构的设计，我们将系统分为展示层、业务逻辑层、数据层。每一层之间为了增加灵活性，我们会添加接口。比如业务逻辑层和数据层之间，我们添加dataservice.purchasedataservice.PurchaseDataService接口。PurchasePO是作为进货记录的持久化对象被添加到设计模型中去的。

　　Purchasedata模块各个类的职责如下所示
<br/>

|模块|职责|
| --- | --- |
|PurchaseData|进货(退货)的模型对象，拥有进货单据的各类数据以及方法|
...
| --- | --- |
|服务名|服务|
|`PurchaseDataService.find(String keywords, FindSaleType type)`|根据keywords查找单一持久化对象|
|`PurchaseDataService.insert(PurchasePO po)`|新建单一持久化对象|
|`PurchaseDataService.delete(String id)`|删除单一持久化对象|
|`PurchaseDataService.update(PurchasePO po)`|更新单一持久化对象|
|`PurchaseDataService.show()`|返回数据库中所有持久化对象|
<br/>
...
　　　　Memberdata模块承担的需求参见需求规格说明文档功能需求几相关非功能需求。
　　　　Memberdata模块的职责及接口参见软件体系结构描述文档表。

　　　　(2)整体结构

　　　　根据体系结构的设计，我们将系统分为展示层、业务逻辑层、数据层。每一层之间为了增加灵活性，我们会添加接口。比如业务逻辑层和数据层之间，我们添加dataservice.memberdataservice.MemberDataService接口。MemberPO是作为客户记录的持久化对象被添加到设计模型中去的。

　　Memberdata模块各个类的职责如下所示
<br/>

|模块|职责|
| --- | --- |
|MemberData|客户的模型对象，拥有客户单据的各类数据以及方法|
...
| --- | --- |
|服务名|服务|
|`MemberDataService.find(String keywords, FindMemberType type)`|根据keywords查找单一持久化对象|
|`MemberDataService.insert(MemberPO po)`|新建单一持久化对象|
|`MemberDataService.delete(String id)`|删除单一持久化对象|
|`MemberDataService.update(MemberPO po)`|更新单一持久化对象|
|`MemberDataService.show()`|返回数据库中所有持久化对象|
<br/>
...
　　　　Promotiondata模块承担的需求参见需求规格说明文档功能需求几相关非功能需求。
　　　　Promotiondata模块的职责及接口参见软件体系结构描述文档表。

　　　　(2)整体结构

　　　　根据体系结构的设计，我们将系统分为展示层、业务逻辑层、数据层。每一层之间为了增加灵活性，我们会添加接口。比如业务逻辑层和数据层之间，我们添加dataservice.promotiondataservice.PromotionDataService接口。PromotionPO是作为促销记录的持久化对象被添加到设计模型中去的。

　　Promotiondata模块各个类的职责如下所示
<br/>

|模块|职责|
| --- | --- |
|PromotionData|促销的模型对象，拥有促销策略单据的各类数据以及方法|
...
| --- | --- |
|服务名|服务|
|`PromotionDataService.find(String keywords, FindPromotionType type)`|根据keywords查找单一持久化对象|
|`PromotionDataService.insert(PromotionPO po)`|新建单一持久化对象|
|`PromotionDataService.delete(String id)`|删除单一持久化对象|
|`PromotionDataService.update(PromotionPO po)`|更新单一持久化对象|
|`PromotionDataService.show()`|返回数据库中所有持久化对象|
<br/>

4.3.11　Classificationdata模块

　　　　(1)模块概述

　　　　Classificationdata模块承担的需求参见需求规格说明文档功能需求几相关非功能需求。
　　　　Classificationdata模块的职责及接口参见软件体系结构描述文档表。

　　　　(2)整体结构

　　　　根据体系结构的设计，我们将系统分为展示层、业务逻辑层、数据层。每一层之间为了增加灵活性，我们会添加接口。比如业务逻辑层和数据层之间，我们添加dataservice.classificationdataservice.ClassificationDataService接口。ClassificationPO是作为分类记录的持久化对象被添加到设计模型中去的。

　　Classificationdata模块各个类的职责如下所示
<br/>

|模块|职责|
| --- | --- |
|ClassificationData|促销的模型对象，拥有促销策略单据的各类数据以及方法|
...
|服务名|服务|
|`ClassificationDataService.find(String name)`|根据name查找单一持久化对象|
|`ClassificationDataService.insert(ClassificationPO po)`|新建单一持久化对象|
|`ClassificationDataService.delete(String id)`|删除单一持久化对象|
|`ClassificationDataService.update(ClassificationPO po)`|更新单一持久化对象|
|`ClassificationDataService.show()`|返回数据库中所有持久化对象|
|`ClassificationDataService.FindClassificationById()`|根据id查找单一持久化对象|
|`ClassificationDataService.getRoot()`|返回分类的根节点|
